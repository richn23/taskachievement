...
4. **Use this exact rung pattern:**
   - **1_mentioned** — a complete sentence names the topic, no reason,
     mechanism, or example provided.
   - **2_developed** — a complete sentence names the topic AND adds a
     reason, cause, effect, or how/why explanation, BUT no concrete
     specific. **The bar at this rung scales with CEFR level.**
   - **3_extended** — all of rung 2 AND a **concrete specific** that
     adds meaningful elaboration, appearing in the SAME sentence or
     directly connected sentences as the reason/explanation.
     **The bar at this rung also scales with CEFR level.**

   **The boundary between 2 and 3 is the concrete specific.** If the
   evidence has a reason but no concrete specific → 2. If it has both
   in the same connected passage → 3. Never split credit across
   distant sentences.

## CRITICAL — task requirements must NOT be knowledge tests

If any of the supplied task requirements demands real-world knowledge
that the student must already possess (e.g., "examples from recent
news", "famous historical events", "named institutions or laws"),
soften the rung descriptors so that **a personal anecdote, a
hypothetical scenario, or a developed-generic example** is equally
valid. Never force the student to cite a specific real-world fact
to score rung 3.

A B2 student in a country with limited media access, or a student
who simply doesn't follow news, must be able to score rung 3 through
personal experience or vivid hypothetical alone.
...
**MANDATORY: at B2 and above, the rung-3 descriptor MUST require ONE of:**

- **TWO meaningful concrete specifics that materially develop the idea**, OR
- **A concrete specific PLUS a comparison/contrast/categorisation**, OR
- **A concrete specific PLUS a qualification/counter-evidence/trade-off**

A single example with no comparison or second development is rung 2 at
B2, not rung 3. This rule is non-negotiable for B2/C1 markers.

**MANDATORY language: every rung-3 descriptor MUST include the phrase
"materially develop" or "materially advance" or equivalent.** Listing
specifics without showing they materially develop the argument is
rung 2, not rung 3. Token-stuffing a name + place + platform without
elaboration does NOT count.

**MANDATORY: rung-3 examples must show development, not just listing.**
A rung-3 worked example like "Facebook to organise reunions and
Instagram to share daily life updates" is platform-listing — that's
rung 2 behaviour. A true rung-3 example shows the platform PLUS what
it enabled and how that materially shapes the argument
(e.g., "Through our Facebook reunion group, we organised our 10-year
class meet-up where 23 of us reconnected — friendships that had gone
silent for a decade are now active again, which shows social media
can revive relationships that distance ended").
...
### B1 example — full ladder for one requirement

INPUT:
TASK REQUIREMENT: "When and why you listen to music"
CEFR LEVEL: B1

OUTPUT for this requirement:

- **1_mentioned**: "states they listen to music with no time, situation,
  or reason given (e.g., 'I listen to music')."

- **2_developed**: "names a specific time/situation AND a specific reason
  for listening (e.g., 'I listen on the bus to work because it makes the
  commute pass faster'). (scores 2, not 3 — names a situation and a
  reason, but no song, artist, or genre that anchors the listening
  habit concretely.)"

- **3_extended**: "all of 2 AND a concrete specific — a song or artist
  (Coldplay, 'Yesterday'), a real genre (gospel, amapiano, classical),
  or a vividly described personal listening situation (e.g., 'every
  Friday evening I play Coldplay on my way home to mark the start of
  the weekend'). Mood categories like 'spiritual songs' or 'motivational
  songs' do NOT count as concrete specifics."
...
## Stress test before returning

Before producing your output, simulate three hypothetical student
responses for each requirement:

- **Weak response** — should clearly score rung 1 against your descriptors
- **Mid response** — should clearly score rung 2 (has reason but no
  concrete specific)
- **Strong response** — should clearly score rung 3 (reason + concrete
  specific)

If any of the three is genuinely ambiguous between two rungs against
your descriptors, REVISE the descriptors before returning. The most
common failure is a vague rung-2/rung-3 boundary — fix this by sharpening
what the rung-3 concrete specific must add beyond the rung-2 reason.

Do NOT include the stress test in the output. It is a private validation step.
