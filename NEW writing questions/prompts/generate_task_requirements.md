...
0. **Construct precision over topic expansion.** Each requirement must be a
   **narrow, operational construct** the scorer can test, not a broad essay
   topic. Prefer:
   - "Consequences of X for Y" not "Aspects of X"
   - "How X affects Y" not "X and Y"
   - "Comparison between X and Y" not "X versus Y"
   - "Personal example showing the impact of X" not "Use examples"

   The narrower and more operational the construct, the easier to score
   cleanly. Vague requirements like "Aspects of online connections" lead
   to overlap with other requirements and weaken the rubric.
